...
def main():
    # Coordenadas Pouso Alegre, MG
    lat = -22.23
    lon = -45.94
    data_dir = Path(__file__).parent / "data"
    raw_path = data_dir / "gfs_pousoalegre_raw.json"
    csv_path = data_dir / "gfs_pousoalegre_hourly.csv"

    print(f"Baixando dados para Pouso Alegre ({lat}, {lon})...")
    try:
        d = fetch_gfs(lat, lon, days=7)
        df = to_dataframe(d)

        # Fetch Marine data (add waves, etc.)
        # Pouso Alegre is inland, but we keep the logic just in case (it might fail or return nulls, or we can wrap in try/except)
        # However, for inland locations, marine API might return error or zeros. 
        # I'll keep the try-except block as in the original script.
        try:
            ...
            print(f"Info: Could not fetch marine data (expected for inland): {e}")
