...
def aggregate_1h(df):
    # Agrega dados horários para blocos de 1h (basicamente reindexa/copia, mas mantém consistência)
    # Como os dados já são horários, basta garantir que as colunas necessárias existam
    # e manter a estrutura de índice
    
    # Para consistência com o resto do código que espera certas agregações (como 'sum' para precipitação em blocos maiores),
    # aqui é 1:1, então a soma de 1h é o próprio valor.
...
def draw_meteogram(df_agg, df_hourly, output_path, city_name, lat, lon, is_litoral):
    ...
    # --- 1. TABELA DE DADOS ---
    # Ajuste para garantir que mostre o sábado inteiro
    # Adicionando margem se necessário, mas o padrão deve funcionar
    ax_table.set_xlim(times[0], times[-1] + pd.Timedelta(hours=1))
    ...
    for i, t in enumerate(times):
        if i > 0:
            ax_table.axvline(t, color=C_GRID, linestyle='--', linewidth=0.5)
            
        x_center = t + pd.Timedelta(minutes=30)
        
        # LINHA 3: Hora e Dia
        hour_txt = t.strftime('%H')
        ax_table.text(x_center, 3.4, hour_txt, ha='center', va='center', fontsize=7, fontweight='bold', color=C_MUTED)
        
        if i == 0 or t.day != times[i-1].day:
            day_txt = t.strftime('%a %d').upper()
            # Ajuste da posição do dia para ficar mais centralizado no bloco do dia, se couber
            # Mas como é só um dia (Sábado), podemos colocar no centro do gráfico ou a cada X horas
            # Vamos simplificar: colocar no início
            ax_table.text(t + pd.Timedelta(hours=12), 3.9, day_txt, ha='center', va='center', fontsize=10, color=C_TEXT, fontweight='bold')
            # if i > 0:
        ...
        cond_type = 'clear'
        if precip >= 0.1: cond_type = 'rain' # Limiar menor para 1h
        ...
        # Texto da condição (Chuva, Limpo) pode ficar muito apertado. 
        # Vamos diminuir a fonte ou abreviar, ou remover se for redundante com o ícone.
        # Vamos tentar fonte bem pequena (5 ou 6)
        ax_table.text(x_center, 2.25, label_txt[:4], ha='center', va='center', fontsize=6, color=label_color, fontweight='bold')

        # LINHA 1: Pressão
        press = df_agg['pressure_msl'].iloc[i]
        ax_table.text(x_center, 1.55, f"{press:.0f}", ha='center', va='center', fontsize=7, color='#8b5cf6', fontweight='bold')
        # Remover "hPa" de cada célula para limpar
        # ax_table.text(x_center, 1.15, "hPa", ha='center', va='center', fontsize=5, color='#8b5cf6')
        ...
        # Reduzir tamanho da seta (s=120 -> s=60)
        ax_table.scatter(x_center, 0.75, marker=m, s=60, color=C_MUTED)
        # Velocidade abaixo da seta
        ax_table.text(x_center, 0.50, f"{w_spd:.0f}", ha='center', va='center', fontsize=7, fontweight='bold', color=C_TEXT)
        
        # Rajada
        c_gust, alpha_gust = get_wind_color(w_gust)
        # Retângulo menor
        rect_w = 1/24 * 0.8 # Largura proporcional a 1h (aprox)
        # Em coordenadas de data, 1h = ? 
        # mdates.date2num converte para float dias. 1h = 1/24 dias.
        w_1h = 1/24.0
        rect_width = w_1h * 0.8
        rect_x = mdates.date2num(x_center) - (rect_width / 2)
        
        ax_table.add_patch(Rectangle((rect_x, 0.1), rect_width, 0.20, color=c_gust, alpha=alpha_gust))
        ax_table.text(x_center, 0.20, f"{w_gust:.0f}", ha='center', va='center', fontsize=6, color=c_gust, fontweight='bold')
    ...
    for i, t in enumerate(times):
        if df_agg['is_day'].iloc[i] == 0:
            ax_temp.axvspan(t, t + pd.Timedelta(hours=1), color='#111827', alpha=0.05)
    ...
    # Barras de chuva
    # Ajuste para 1h: 1h = 1/24 dias ~= 0.0416
    bar_width = 0.035 
    offset = 0.5 / 24 # Centralizar em 30min
    
    ax_rain.bar(mdates.date2num(times) + offset, df_agg['precipitation'], width=bar_width, color=C_RAIN, alpha=0.5, label='Chuva')
    
    for t, val in zip(times, df_agg['precipitation']):
        if val > 0.1:
            # Texto menor e mais próximo da barra
            ax_rain.text(t + pd.Timedelta(minutes=30), val + 0.1, f"{val:.1f}", ha='center', va='bottom', fontsize=6, color=C_RAIN, fontweight='bold', bbox=dict(facecolor='white', alpha=0.6, edgecolor='none', pad=0.5))
            
    ax_rain.set_ylim(0, max(10, df_agg['precipitation'].max() * 1.5))
...
def main():
    ...
    for city in cities:
        ...
        # Alterado nome do arquivo de saída
        output_path = root / f"meteograma_{file_name}_sabado.png"
        ...
        try:
            ...
            # Salvar backup (Opcional: podemos não salvar para não sobrescrever o oficial, 
            # mas como é o mesmo dado, não tem problema atualizar o cache)
        ...
        # 2. Filtragem para Sábado
        print("\n--- Etapa 1.5: Filtrando para Sábado ---")
        try:
            today = pd.Timestamp.now().normalize()
            # 0=Mon, 1=Tue, ..., 5=Sat, 6=Sun
            # Se hoje é sexta (4), faltam 1 dia. (5-4)%7 = 1
            # Se hoje é sábado (5), faltam 0 dias. (5-5)%7 = 0
            days_until_saturday = (5 - today.dayofweek) % 7
            
            # Se quisermos garantir que seja o PRÓXIMO sábado caso hoje já seja sábado e tarde
            # mas vamos manter a lógica simples: próximo sábado (ou hoje se for sábado)
            
            target_date = today + pd.Timedelta(days=days_until_saturday)
            start_time = target_date
            end_time = target_date + pd.Timedelta(hours=23, minutes=59)
            
            print(f"Filtrando dados para: {target_date.strftime('%d/%m/%Y')}")
            
            df_full = df.copy() # Backup se precisar
            df = df[(df.index >= start_time) & (df.index <= end_time)]
            
            if df.empty:
                print("AVISO: DataFrame vazio após filtro de data. Verifique se os dados baixados cobrem o próximo sábado.")
                # Fallback: Se não tem dados para o próximo sábado (ex: estamos no domingo e o forecast é só 7 dias, mas sábado que vem é dia 7? Não, 6 dias. Deve ter.)
                # Se estamos na segunda, sábado é d+5. O forecast tem 7 dias. Deve ter.
                # Se algo der errado, vamos abortar
                continue
        ...
        # 3. Processamento e Geração do Gráfico
        print("\n--- Etapa 2: Geração do Meteograma (Sábado) ---")
        try:
            if df is not None and not df.empty:
                # Usar aggregate_1h ao invés de aggregate_12h
                df_1h = aggregate_1h(df)
                draw_meteogram(df_1h, df, output_path, city["name"], lat, lon, is_litoral)
            else:
